...
public class SOInstanceAttribute : PropertyAttribute { }
[CustomPropertyDrawer(typeof(SOInstanceAttribute), true)]
public class SOInstanceDrawer : PropertyDrawer {
	...
	private void TryRemoveSOInstance(SerializedProperty property, ScriptableObject so) {
		if ((property.serializedObject.targetObject as Component).gameObject.scene.name == null && AssetDatabase.IsSubAsset(so)) {
			AssetDatabase.RemoveObjectFromAsset(so);
			AssetDatabase.SaveAssets();
			GUI.changed = true;
		}
	}
	private void TryAddSOInstance(SerializedProperty property, ScriptableObject so, string name) {
		var prefab = (property.serializedObject.targetObject as Component).gameObject;
		if (prefab.scene.name == null) {
			so.name = name;
			AssetDatabase.AddObjectToAsset(so, prefab);
			AssetDatabase.SaveAssets();
			GUI.changed = true;
		}
	}

	private void DrawUniversalField(Rect position, SerializedProperty property, Type propertyType) {
		var oldValue = property.objectReferenceValue;
		var value = EditorGUI.ObjectField(position, oldValue, typeof(Object), false);
		if (oldValue != value) {
			if (oldValue) TryRemoveSOInstance(property, (ScriptableObject)oldValue);
			if (value == null) {
				property.objectReferenceValue = null;
			} else if (value is MonoScript) {
				var obj = ScriptableObject.CreateInstance(value.name);
				if (propertyType.IsAssignableFrom(obj.GetType())) {
					TryAddSOInstance(property, obj, value.name);
					property.objectReferenceValue = obj;
				}
			} else if (propertyType.IsAssignableFrom(value.GetType())) {
				var option = EditorUtility.DisplayDialogComplex("SOInstance", "ScriptableObject as ...", "Original", "Cancel", "Instance");
				if (option == 0) {
					property.objectReferenceValue = value;
				} else if (option == 2) {
					var obj = (ScriptableObject)GameObject.Instantiate(value);
					TryAddSOInstance(property, obj, value.name);
					property.objectReferenceValue = obj;
				}
			}
	...
	public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
		...
		DrawUniversalField(propertyRect, property, type);
		...
		if (property.propertyType == SerializedPropertyType.ObjectReference && property.objectReferenceValue != null) {
			...
			if (GUI.Button(buttonRect, "Create")) {
				var option = EditorUtility.DisplayDialogComplex("SOInstance", "ScriptableObject as ...", "Asset", "Cancel", "Instance");
				if (option == 0) {
					string selectedAssetPath = "Assets";
					if (property.serializedObject.targetObject is MonoBehaviour) {
						MonoScript ms = MonoScript.FromMonoBehaviour((MonoBehaviour)property.serializedObject.targetObject);
					...
					TryAddSOInstance(property, obj, type.Name);
					property.objectReferenceValue = obj;
				}
				property.serializedObject.ApplyModifiedProperties();
				GUIUtility.ExitGUI();
